...
def main():
    ...
    prev_time = time.time()
    processing_latency = 0.0  # ADDED: Initialize processing latency
    ...
    # MODIFIED: Drastically reduced threshold to allow faster transition to predictive aiming.
    ...
    try:
        while True:
            t_loop_start = time.time()  # ADDED: Mark the start of the processing loop
            ...
            if frame_exposed_current_loop is not None:
                # MODIFIED: Pass the measured processing_latency to the tracking function
                processed_display_frame, active_tracks_list, locked_target_info = process_frame_with_tracking(
                    ...
                    processing_latency  # Pass the latency from the *previous* frame
                )
            ...
            if display_frame is not None:
                ...
                # MODIFIED: Display text is now in the bottom-left corner
                cv2.putText(display_frame, tracking_mode_text, (10, display_frame.shape[0] - 10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, text_color, 2, cv2.LINE_AA)

                cv2.putText(display_frame, f"Exp: {exposure_factor:.1f}", (display_frame.shape[1] - 120, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2, cv2.LINE_AA)
                if is_paused:
                    cv2.putText(display_frame, "PAUSED", (display_frame.shape[1] // 2 - 50, 30),
            ...
            # ADDED: Calculate the processing latency for the *next* frame at the end of the loop
            processing_latency = time.time() - t_loop_start
