...
# MODIFIED: Import estimate_distance from config now
from config import estimate_distance
...
def calculate_track_score(track, frame_width, frame_height):
    ...
    # 2. Angle Offset Score
    angle_score = 0.0
    if MAX_NORMALIZED_CENTER_DIST > 0:
    ...
    size_score = 0.0
    if MAX_NORMALIZED_AREA > 0:
        size_score = norm_track_area / MAX_NORMALIZED_AREA
...
def select_best_target(active_tracks, current_locked_track_id, frame_width, frame_height):
    ...
    locked_track_obj = None
    locked_track_score_unmodified = -1.0
...
# MODIFIED: Added 'processing_latency_s' to the function signature
def process_frame_with_tracking(model, frame_raw_bgr, frame_for_detection_bgr, active_tracks, labels_dict, infer_cfg,
    ...
    surviving_tracks = [t for t in active_tracks if not t.is_lost]
    if len(surviving_tracks) < len(active_tracks):
        lost_track_ids = {t.track_id for t in active_tracks if t.is_lost}
        if g_locked_target_track_id in lost_track_ids:
            g_locked_target_track_id = None
    ...
    for track in active_tracks:
        ...
        # MODIFIED: Dynamic prediction logic
        bullet_flight_time_s = 0.0
        if track.estimated_distance is not None:
            bullet_flight_time_s = track.estimated_distance / cfg.BULLET_SPEED_MPS

        total_prediction_time_s = processing_latency_s + bullet_flight_time_s
        track.calculate_aim_point(total_prediction_time_s)  # Pass total time to predict
        ...
        if track.track_id == g_locked_target_track_id:
            ...
            if track.predicted_aim_point:
                aim_x_clamped = np.clip(track.predicted_aim_point[0], 0, raw_img_w - 1)
                aim_y_clamped = np.clip(track.predicted_aim_point[1], 0, raw_img_h - 1)
                normalized_aim_point = (aim_x_clamped / raw_img_w, aim_y_clamped / raw_img_h)

                center_x_clamped = np.clip(track.center_x, 0, raw_img_w - 1)
                center_y_clamped = np.clip(track.center_y, 0, raw_img_h - 1)
        ...
        if track.predicted_aim_point:
            aim_x_clamped = np.clip(track.predicted_aim_point[0], 0, display_frame.shape[1] - 1)
            aim_y_clamped = np.clip(track.predicted_aim_point[1], 0, display_frame.shape[0] - 1)
    ...
    cv2.putText(display_frame, f"FPS: {fps:.1f} Tracks: {len(active_tracks)}",
