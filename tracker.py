...
class Track:
    ...
    # MODIFIED: This function is now the core of the prediction logic.
    def calculate_aim_point(self, prediction_time_s):
        """
        Calculates the predicted aim point based on current velocity and a given prediction time.
        :param prediction_time_s: Total time (in seconds) to predict into the future.
        """
        ...
        if velocity_magnitude_sq < cfg.MIN_VELOCITY_FOR_PREDICTION_SQ:
            ...
            # The Kalman Filter's velocity state (curr_vx) is in pixels per dt (cfg.KF_DT).
            # We need to convert it to pixels per second.
            # velocity_pixels_per_second = velocity_pixels_per_dt / dt_in_seconds
            vx_pixels_per_second = curr_vx / cfg.KF_DT
            vy_pixels_per_second = curr_vy / cfg.KF_DT

            # Predict aim point: current position + (velocity * total_prediction_time)
            aim_x = curr_x + vx_pixels_per_second * prediction_time_s
            aim_y = curr_y + vy_pixels_per_second * prediction_time_s
