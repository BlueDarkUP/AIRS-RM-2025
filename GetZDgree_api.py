...
def calculate_object_rotation(bbox_width: float, bbox_height: float) -> float:
    ...
    if bbox_short_side < epsilon:  # Avoid division by zero
        return -1.0
    ...
    if abs(denominator) < epsilon:
        # If denominator is near zero, it implies a very large or undefined tan value,
        # often meaning the angle is close to 90 degrees.
        # However, the context of atan is usually safe. Let's handle it carefully.
        if abs(numerator) < epsilon:
            alpha_rad = 0.0  # 0/0 case
        else:
            # A large value, effectively Pi/2
            alpha_rad = math.pi / 2 if numerator > 0 else -math.pi / 2
    else:
